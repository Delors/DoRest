...
final private[httpd] class HConn(
  ...
  private def inHeader : HReqState.Value = if (tail.length < 28) HReqState.WaitsForHeader else {
    
    val till = tail.length - 4
    ...
    findEol(24) match {
      ...
      case Some(shift) => if (shift >= maxHeaderLength) HReqState.IsInvalid else {
        ...
        tail = tail.slice(shift + 4, tail.length)
  ...
  private def inData : HReqState.Value = {
    ...
    else if (length > tail.length) HReqState.WaitsForData else {
  ...
  private def inParts : HReqState.Value = {
    ...
    def acceptResult(finalState : HReqState.Value) : HReqState.Value = {
      acceptedTotalLength += tail.length
    ...
    appLet.get._2.partsAcceptor(headerData) match {
      ...
      case Some(acceptor) => 
        if (parts.isEmpty) {
          ...
          if (tail.length >= minSize) parts = Some(new HParts(acceptor, headerData))
        }
